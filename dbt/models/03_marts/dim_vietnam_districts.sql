...
deduplicated as (
    ...
    from (
        select *,
            row_number() over (partition by district_id order by updated_at desc) as row_num
        from transformed
    )
    where row_num = 1
)
...
{% if is_incremental() %} 
    ...
    select  distinct
            ...
            from deduplicated  
            left join {{ this }} as existing on deduplicated.district_id = existing.district_id 
            where 
